...
test("picker press interception protects the page before its handlers run", () => {
  ...
  // unless the panel's relate flow owns the click, which picks a target.
  assert.match(picker, /\{ kind: "select", additive: event\.shiftKey \}/);
  assert.match(picker, /state\.relateSourcePinId !== null/);
