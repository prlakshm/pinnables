...
fixtures/
  film-set/            flowers herbarium demo for recording (port 5181)
  demo-app/            card-consistency demo (port 5180)
examples/acme-app/     React target for the dashboard-cards board (port 5173)
scripts/               MCP smoke test, serve-demo, wordmark tracer
```

## Flowers website (localhost:5181)

The flowers set is `fixtures/film-set` — **Accession**, a birth-month flowers register. Start it with:

```bash
npm run film
```

Then load the unpacked extension from `packages/extension/dist` at `chrome://extensions`, start the
companion service (`npm run dev:service`), and open http://localhost:5181. Click the Pinnables
toolbar icon to open the side panel and arm capture mode.

```bash
npm install && npm run build
npm run dev:service    # terminal 1 — writes boards to ~/.pinnables
npm run film           # terminal 2 — flowers site on :5181
npm run dev:extension  # terminal 3 — rebuild extension on change (after first build)
```
